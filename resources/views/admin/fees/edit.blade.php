...
<div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
    <h2 class="text-xl font-semibold mb-6">Edit Fee</h2>

    <form action="{{ route('admin.fees.update', $fee->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="student_id" class="h-12 px-3 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">
                @foreach($students as $s)
                    <option value="{{ $s->id }}" {{ old('student_id', $fee->student_id)==$s->id ? 'selected' : '' }}>{{ $s->name }} (ID: {{ $s->id }})</option>
                @endforeach
            </select>
    ...
    {{-- Fee Type --}}
        <div class="relative w-full">
            ...
            {{-- Hidden input (array) --}}
            <input type="hidden" name="fee_type" id="fee_type_input" value="{{ old('fee_type', is_array($fee->fee_type) ? implode(',', $fee->fee_type) : $fee->fee_type) }}">
            ...
            <div id="fee-dropdown" class="hidden absolute mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg z-10">
                <div class="flex items-center px-4 py-2 hover:bg-gray-100">
                    <input type="checkbox" id="fee-hostel" value="hostel_rent" class="mr-2" onchange="updateFeeSelected()"
                           {{ (old('fee_type') ? in_array('hostel_rent', explode(',', old('fee_type'))) : (is_array($fee->fee_type) && in_array('hostel_rent', $fee->fee_type))) ? 'checked' : '' }}>
                ...
                <div class="flex items-center px-4 py-2 hover:bg-gray-100">
                    <input type="checkbox" id="fee-mess" value="mess_fee" class="mr-2" onchange="updateFeeSelected()"
                           {{ (old('fee_type') ? in_array('mess_fee', explode(',', old('fee_type'))) : (is_array($fee->fee_type) && in_array('mess_fee', $fee->fee_type))) ? 'checked' : '' }}>
                ...
                <div class="flex items-center px-4 py-2 hover:bg-gray-100">
                    <input type="checkbox" id="fee-japanese" value="japanese_course" class="mr-2" onchange="updateFeeSelected()"
                           {{ (old('fee_type') ? in_array('japanese_course', explode(',', old('fee_type'))) : (is_array($fee->fee_type) && in_array('japanese_course', $fee->fee_type))) ? 'checked' : '' }}>
                ...
                <div class="flex items-center px-4 py-2 hover:bg-gray-100">
                    <input type="checkbox" id="fee-security" value="security_deposit" class="mr-2" onchange="updateFeeSelected()"
                           {{ (old('fee_type') ? in_array('security_deposit', explode(',', old('fee_type'))) : (is_array($fee->fee_type) && in_array('security_deposit', $fee->fee_type))) ? 'checked' : '' }}>
                ...
                <div class="flex items-center px-4 py-2 hover:bg-gray-100">
                    <input type="checkbox" id="fee-other" value="other" class="mr-2" onchange="updateFeeSelected()"
                           {{ (old('fee_type') ? in_array('other', explode(',', old('fee_type'))) : (is_array($fee->fee_type) && in_array('other', $fee->fee_type))) ? 'checked' : '' }}>
        ...
        <div>
            ...
            <input type="number" step="0.01" name="amount" value="{{ old('amount', $fee->amount) }}" class="h-12 px-3 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">
        ...
        <div>
            ...
            <input type="date" name="due_date" value="{{ old('due_date', optional($fee->due_date)->format('Y-m-d')) }}" class="h-12 px-3 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">
        ...
        <div>
            ...
            <input type="date" name="paid_date" value="{{ old('paid_date', optional($fee->paid_date)->format('Y-m-d')) }}" class="h-12 px-3 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">
        ...
        <div>
            ...
            <input type="number" step="0.01" name="paid_amount" value="{{ old('paid_amount', $fee->paid_amount) }}" class="h-12 px-3 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="payment_method" value="{{ old('payment_method', $fee->payment_method) }}" class="h-12 px-3 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">
            ...
            <div>
                ...
                <input type="text" name="receipt_number" value="{{ old('receipt_number', $fee->receipt_number) }}" class="h-12 px-3 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">
        ...
        <div>
            ...
            <textarea name="notes" rows="3" class="px-3 py-2 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">{{ old('notes', $fee->notes) }}</textarea>
...
// Initialize selected fees on page load
