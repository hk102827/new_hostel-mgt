...
@section('title', 'Edit Fees Collection')
@section('page-title', 'Edit Fees Collection')

@section('content')
<div class="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow-md">
    <div class="text-center mb-8">
        <h2 class="text-2xl font-bold text-gray-800">Edit Fees Collection</h2>
    ...
    <form action="{{ route('admin.fees.update', $fee->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                   ...
                   <div>
                    ...
                    <input type="month" 
                           name="fees_month" 
                           id="fees_month"
                           value="{{ old('fees_month', $fee->fees_month) }}" 
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border border-gray-300 px-2 py-1">
                            <input type="number" 
                                   ...
                                   id="monthly_fee" 
                                   step="0.01" 
                                   value="{{ old('monthly_fee', $fee->monthly_fee) }}"
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border border-gray-300 px-2 py-1">
                            <input type="number" 
                                   ...
                                   id="admission_fee" 
                                   step="0.01" 
                                   value="{{ old('admission_fee', $fee->admission_fee) }}"
                                   class="w-full h-8 px-2 text-center border-0 focus:ring-0 text-sm" 
                                   onchange="calculateTotal()">
                        </td>
                    </tr>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border border-gray-300 px-2 py-1">
                            <input type="number" 
                                   ...
                                   id="hostel_fee" 
                                   step="0.01" 
                                   value="{{ old('hostel_fee', $fee->hostel_fee) }}"
                                   class="w-full h-8 px-2 text-center border-0 focus:ring-0 text-sm" 
                                   onchange="calculateTotal()">
                        </td>
                    </tr>
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border border-gray-300 px-2 py-1">
                            <input type="number" 
                                   ...
                                   step="0.01" 
                                   value="{{ old('previous_month_fee', $fee->previous_month_fee) }}"
                                   class="w-full h-8 px-2 text-center border-0 focus:ring-0 text-sm" 
                                   onchange="calculateTotal()"
                                   readonly>
                        </td>
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="flex justify-between items-center">
                    ...
                    <span id="total_amount" class="font-bold text-lg">{{ $fee->total_amount }}</span>
                    <input type="hidden" name="total_amount" id="total_amount_input" value="{{ $fee->total_amount }}">
                </div>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span id="due_balance" class="font-bold text-lg text-red-600">{{ $fee->due_balance }}</span>
                    <input type="hidden" name="due_balance" id="due_balance_input" value="{{ $fee->due_balance }}">
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="payment_method" class="h-10 px-3 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                    ...
                    <option value="bank" {{ old('payment_method', $fee->payment_method)=='bank' ? 'selected' : '' }}>Bank Transfer</option>
        ...
        <div>
            ...
            <textarea name="notes" 
                      ...
                      placeholder="Add any additional notes...">{{ old('notes', $fee->notes) }}</textarea>
        </div>

        <!-- Actions -->
        <div class="flex justify-center gap-4">
            <a href="{{ route('admin.fees.index') }}" 
               class="px-8 py-3 bg-gray-500 text-white rounded-lg hover:bg-gray-600 font-medium">
                Cancel
            </a>
            <button type="submit" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    calculateTotal();
    calculateBalance(); // Also calculate balance to set proper color
});
