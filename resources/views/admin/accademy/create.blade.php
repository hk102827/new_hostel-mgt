...
@section('title', 'Add Academy Student')
@section('page-title', 'Create Academy Student')

@section('content')
<div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
    <h2 class="text-xl font-semibold mb-6">Add New Japanese Academy Student</h2>

    <form action="{{ route('admin.academy.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <select name="student_type" class="h-12 px-3 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">
                ...
                <option value="online" {{ old('student_type') == 'online' ? 'selected' : '' }}>Online</option>
                <option value="physical" {{ old('student_type') == 'physical' ? 'selected' : '' }}>Physical</option>
            </select>
            @error('student_type')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <select name="hostel" class="h-12 px-3 block w-full rounded-lg border-gray-300 shadow-md focus:border-indigo-500 focus:ring-indigo-500 text-base">
                <option value="0" {{ old('hostel', '0') == '0' ? 'selected' : '' }}>No</option>
                <option value="1" {{ old('hostel') == '1' ? 'selected' : '' }}>Yes</option>
            </select>
            @error('hostel')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
